...
-- One-release compatibility forwards; Planner modules own cache and demand planning.
function RS.ClearCountCaches()
    return StockPiler2.Planner.SpecHaveCache.ClearCountCaches()
end

function RS.BeginPlanCraftsMemo()
    return StockPiler2.Planner.SpecHaveCache.BeginPlanCraftsMemo()
end

function RS.WarmSpecHaveCache(specs)
    return StockPiler2.Planner.SpecHaveCache.WarmSpecHaveCache(specs)
end
...
function RS.CollectAutoGrowSeedLines()
    return StockPiler2.Planner.SpecDemand.CollectAutoGrowSeedLines()
end

function RS.CollectAutoGrowFocus()
    return StockPiler2.Planner.SpecDemand.CollectAutoGrowFocus()
end

function RS.CollectAutoBuyFocus()
    return StockPiler2.Planner.SpecDemand.CollectAutoBuyFocus()
end

function RS.FocusSpecKeys(focus)
    return StockPiler2.Planner.SpecDemand.FocusSpecKeys(focus)
end

function RS.FocusWatchNames(focus)
    return StockPiler2.Planner.SpecDemand.FocusWatchNames(focus)
end

function RS.ShouldAutoGrowPotion(potionKey, watch)
    return StockPiler2.Planner.SpecDemand.ShouldAutoGrowPotion(potionKey, watch)
end

function RS.BuildBalancedSpecDemand()
    return StockPiler2.Planner.SpecDemand.BuildBalancedSpecDemand()
end
...
function RS.CountCraftsPossible(recipe, opts)
    ...
    if respectReserve ~= true then
        ...
        local HC = StockPiler2.Planner and StockPiler2.Planner.SpecHaveCache
        local memo = HC and HC.GetCraftsPossibleMemo and HC.GetCraftsPossibleMemo() or nil
    ...
    if type(slots) ~= "table" or #slots == 0 then
        local HC = StockPiler2.Planner and StockPiler2.Planner.SpecHaveCache
        if memoKey ~= nil and memoKey ~= "" and HC and HC.SetCraftsPossibleMemo then
            HC.SetCraftsPossibleMemo(memoKey, 0)
        end
        return 0
    end
    local possible = nil
    ...
    local HC = StockPiler2.Planner and StockPiler2.Planner.SpecHaveCache
    if memoKey ~= nil and memoKey ~= "" and HC and HC.SetCraftsPossibleMemo then
        HC.SetCraftsPossibleMemo(memoKey, result)
    end
    return result
end
